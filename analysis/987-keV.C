{
    ...
    int firstRun= 9963, lastRun= 9974, goodpoints=0; // 5 Torr
    ...
    vector<int> runs;
     ...
     for (int i = firstRun; i<=lastRun; i++){
      if((i==9967 || i==9968 || i==9969 || i==9970 || i==9972 || i==9973)){
        continue;
      }
      runs.push_back(i);
    }
   ...
   // dragon::MakeChains(runs, "/data2/dragon/S1843/data/rootfiles/run%d.root");
    ...
    TCut bgo = "head.bgo.esort[0]>1.0 && head.bgo.esort[0]<11.0";
    //TCut SepTOF = "!(tail.io32.trigger_latch & 128) && !(coinc.xtofh==-1)";
    TCut pulser = "!(tail.io32.trigger_latch & 128) && !(coinc.xtofh==-1)";
    TCut recoils = "coinc.xtofh>1700 && coinc.xtofh<1900";
    TCut dsssd = "tail.dsssd.efront>200 && tail.dsssd.efront<600";
    ...
    // Plots
      ...
      c4->SetTitle("E_{c.m.} = 987 keV - BGO Hit");
      c4->SaveAs("987-keV/BGO_hit0.pdf");
    ...
    //BGO E
      ...
      h_bgo->SetTitle("E_{c.m.} = 987 keV - #gamma_{0} Energy Spectrum");
      h_bgo->GetYaxis()->SetTitle("Counts [50 keV / bin]");
      h_bgo->GetXaxis()->SetTitle("#gamma_{0} Energy [MeV]");
      c0->SaveAs("987-keV/987_BGO_E.pdf");
...
// Graph Legend Settings
    ...
    */
    ...
    singles->SetStats(0);
    singles->GetXaxis()->SetRangeUser(0,900);
    ...
    c2->SaveAs("987-keV/987_DSSSD_E.pdf");
    ...
    c14->SaveAs("987-keV/987_dsssdXY.pdf");
    ...
    c30->SaveAs("987-keV/987_sepTOF.pdf");
    ...
    TH2F *icdsssd = new TH2F("icdsssd", "icdsssd", 100, 0, 1000, 1e3, 0, 4e3);
    ...
    TH2F *icdsssd1 = new TH2F("icdsssd1", "icdsssd1", 100, 0, 1000, 1e3, 0, 1e4);
    icdsssd1->SetMarkerStyle(21);
    icdsssd1->SetMarkerColor(2);
    icdsssd1->SetMarkerSize(0.6);

    icdsssd1->SetTitle("IC sum vs DSSSD energy - E_{c.m} = 987 keV");
    ...
    c13->SaveAs("987-keV/987_icdsssd_E.pdf");
}
